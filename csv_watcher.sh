...
if [ -f "$SCRIPT_DIR/lib/antivirus.sh" ]; then
...
verifier_systeme() {
    ...
    for service in "${services_critiques[@]}"; do
        if ! systemctl is-active --quiet "$service" 2>"$DEV_NULL"; then
            log_alert "Service critique $service inactif - tentative redémarrage"
            systemctl restart "$service" 2>"$DEV_NULL" || log_error "Échec redémarrage $service"
        fi
    done
    ...
    if [ "$disk_usage" -gt 95 ] 2>"$DEV_NULL"; then
        log_alert "Espace disque critique: ${disk_usage}% - Nettoyage d'urgence"
        find /tmp -type f -mtime +1 -delete 2>"$DEV_NULL" || true
        find /var/log -name "*.log" -size +100M -exec truncate -s 50M {} \; 2>"$DEV_NULL" || true
    fi
}
...
# Vérifier que inotifywait est disponible
    if ! commande_existe inotifywait; then
        ...
        apt-get update >"$DEV_NULL" 2>&1
        apt-get install -y inotify-tools >"$DEV_NULL" 2>&1
    fi
...
while true; do
    ...
    if inotifywait -m -e modify,create,delete,move --timeout 600 "$CSV_FILE" 2>"$DEV_NULL" | while read -r path action file; do
        echo "[$(date '+%Y-%m-%d %H:%M:%S')] CSV $action: $file" >> /var/log/csv_changes.log

        # Attendre que l'écriture soit terminée (avec timeout)
        local timeout=30
        local count=0
        while [ $count -lt $timeout ] && lsof "$CSV_FILE" >"$DEV_NULL" 2>&1; do
            sleep 1
            ((count++))
        done
