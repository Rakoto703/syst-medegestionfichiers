...
# Fallback pour /dev/null si le système a un /dev en lecture seule ou protégé
if [ -c /dev/null ] && : > /dev/null 2>&1; then
    export DEV_NULL="/dev/null"
else
    mkdir -p "$TMP_DIR" 2>/dev/null || true
    export DEV_NULL="$TMP_DIR/null"
    if ! : > "$DEV_NULL" 2>/dev/null; then
        export DEV_NULL="/dev/null"
    fi
fi
