...
[global]
   ...
   # Leave interface selection to Samba and the system network stack for stability
...
redemarrer_samba() {
    if commande_existe systemctl; then
        ...
        # Si nmbd existe, redémarrer seulement s'il est actif pour éviter d'instabiliser le service.
